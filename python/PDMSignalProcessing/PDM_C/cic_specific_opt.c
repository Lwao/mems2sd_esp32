        ...
        // while(num_samples = fread(buffer, BYTE_PER_SAMPLE, SAMPLES, fileb) == SAMPLES)
        {
            ...
            for(int ii=0; ii<num_samples; ii++)
            {
                ...
                for(int jj=0; jj<2; jj++)
                {
                    ...
                    pcm_samples[2*ii+jj] = temp - cic.prev_s2;
                    cic.prev_s2 = temp;

                    pcm_samples[2*ii+jj] = pcm_samples[2*ii+jj]>=MAX_OVERFLOW ? pcm_samples[2*ii+jj]-MAX_OVERFLOW : pcm_samples[2*ii+jj];
                    pcm_samples[2*ii+jj] = pcm_samples[2*ii+jj]<=MIN_OVERFLOW ? pcm_samples[2*ii+jj]-MIN_OVERFLOW : pcm_samples[2*ii+jj];
                }
            }
